...
from config import MAX_TRIES, DRS_DB_PATH
...
def create_drs_object(obj, tries=1):
    ...
    try:
        with Session() as session:
            ...
            new_object.self_uri = f'{os.getenv("DRS_URL").replace("http://", "drs://").replace("https://", "drs://")}/{new_object.name}'
    ...
    with Session() as session:
        ...
        for row in session.execute(stmt):
            ...
            if result["description_1"] == "raw_reads":
                ...
                if "verified" in result["meta_data"] and result["meta_data"]["verified"] == 1:
